...
The gateway exposes an aggregation endpoint at `/api/aggregation/person/{id}` returning combined order, profile and organization data. Prometheus metrics from each service are available at `/metrics`. The Prometheus exporter package is pinned to **1.11.0** as newer releases dropped .NET 6 support. The profile and organization services now expose full CRUD APIs via `/api/profile` and `/api/org`.
